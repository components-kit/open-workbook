...
export type ToolNamespace = CapabilityNamespace;
export type ToolContract = CapabilityContract;
export type ToolCatalogOptions = CapabilityCatalogOptions;
...
export const InternalCapabilityCatalog: CapabilityContract[] = TOOL_NAMES.map((name) => {
  ...
  return {
    ...
    title: titleize(name),
    namespace,
    status: getToolStatus(name),
    mutatesWorkbook,
    destructiveLevel,
    requiresConfirmation: mutatesWorkbook,
    requiredCapabilities: getRequiredCapabilities(name, namespace),
    description: `Open Workbook ${name} internal capability.`
  };
});

export const PublicAgentToolCatalog = InternalCapabilityCatalog.filter((capability) => capability.name === "excel.agent.run");

// Compatibility aliases for older internal imports. Prefer capability naming for backend inventory
// and public-agent-tool naming for the MCP tools/list surface.
export const ToolCatalog = InternalCapabilityCatalog;
export const PublicToolCatalog = PublicAgentToolCatalog;
export const InitialToolContracts = getPublicAgentToolCatalog();

export function getPublicAgentToolCatalog(_options: CapabilityCatalogOptions = {}): CapabilityContract[] {
  return PublicAgentToolCatalog;
}

export function getExposedToolCatalog(options: CapabilityCatalogOptions = {}): CapabilityContract[] {
  return getPublicAgentToolCatalog(options);
}

export function getInternalCapabilityCatalog(options: CapabilityCatalogOptions = {}): CapabilityContract[] {
  return InternalCapabilityCatalog.filter((capability) => {
...
export function getPublicAgentToolCatalogSummary(options: CapabilityCatalogOptions = {}) {
  const exposed = getPublicAgentToolCatalog(options);
  return {
    total: PublicAgentToolCatalog.length,
    exposed: exposed.length,
    stable: PublicAgentToolCatalog.filter((tool) => tool.status === "stable").length,
    preview: PublicAgentToolCatalog.filter((tool) => tool.status === "preview").length,
    planned: PublicAgentToolCatalog.filter((tool) => tool.status === "planned").length,
    unsupported: PublicAgentToolCatalog.filter((tool) => tool.status === "unsupported").length,
    tools: PublicAgentToolCatalog,
    internalCapabilities: getInternalCapabilityCatalogSummary(options)
  };
}

export function getToolCatalogSummary(options: CapabilityCatalogOptions = {}) {
  return getPublicAgentToolCatalogSummary(options);
}

export function isPublicAgentTool(name: string, options: CapabilityCatalogOptions = {}): boolean {
  return getPublicAgentToolCatalog(options).some((tool) => tool.name === name);
}

export function isToolExposed(name: string, options: CapabilityCatalogOptions = {}): boolean {
  return isPublicAgentTool(name, options);
}

export function getInternalCapabilityCatalogSummary(options: CapabilityCatalogOptions = {}) {
  const capabilities = getInternalCapabilityCatalog(options);
  return {
    total: InternalCapabilityCatalog.length,
    exposed: 0,
    stable: InternalCapabilityCatalog.filter((capability) => capability.status === "stable").length,
    preview: InternalCapabilityCatalog.filter((capability) => capability.status === "preview").length,
    planned: InternalCapabilityCatalog.filter((capability) => capability.status === "planned").length,
    unsupported: InternalCapabilityCatalog.filter((capability) => capability.status === "unsupported").length,
    capabilities
  };
}

function getNamespace(name: string): CapabilityNamespace {
  const namespace = name.split(".")[1];
  if (!namespace) {
    throw new Error(`Invalid tool name: ${name}`);
  }
  return namespace as CapabilityNamespace;
}

function getToolStatus(name: string): CatalogStatus {
...
function getDestructiveLevel(name: string, namespace: CapabilityNamespace): DestructiveLevel {
...
function getRequiredCapabilities(name: string, namespace: CapabilityNamespace): string[] {
