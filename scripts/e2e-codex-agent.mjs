...
async function runScenario(scenario, iteration) {
  ...
  const codexProcess = spawn(
    ...
    [
      ...
      "-c",
      `mcp_servers.open-workbook.env={OPEN_WORKBOOK_HOST="127.0.0.1",OPEN_WORKBOOK_PORT="${backendPort}",OPEN_WORKBOOK_ADDIN_PATH="/addin",OPEN_WORKBOOK_DISABLE_UPDATE_CHECK="1",OPEN_WORKBOOK_STATE_DIR="${path.join(tempRoot, scenarioSlug, "state")}",OPEN_WORKBOOK_BACKUP_DIR="${path.join(tempRoot, scenarioSlug, "backups")}"}`,
      "-c",
