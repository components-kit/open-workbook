...
export function agentRunInputSchema() {
  ...
  const intentSchema = jsonObjectString(z.object({
    action: z.string().optional().describe("Optional internal action hint, such as read_values, read_formulas, write_values, derive_values, validate_workbook, or create_pivot_chart_summary. Invalid hints are ignored by the backend."),
