...
- `corepack pnpm test:e2e` passes before deterministic E2E claims. This runs MCP contract, workbook fixture, scenario fixture contract, default agent surface smoke, default fake-host workflow smoke, the strict stable MCP scenario subset, the strict worksheet regression scenario pack, and the strict department workflow scenario pack without a real LLM.
...
- `corepack pnpm test:e2e:mcp-contract` passes before MCP protocol compatibility claims. This lane is also included in `corepack pnpm test:e2e`.
- `corepack pnpm test:e2e:workbook` passes before claiming CI-safe OOXML fixture assertion coverage for workbook file parts after a local operation plan.
- `corepack pnpm test:e2e:scenario-contract` passes before claiming regression or department scenario fixture completeness. This lane ensures scenarios include budgets, route assertions, safety expectations, workbook-output assertions for mutating cases, disconnected-workbook setup for recovery cases, and required department/operation coverage.
- `corepack pnpm test:e2e:scenarios` passes before claiming deterministic MCP scenario coverage for the current strict subset.
- `corepack pnpm test:e2e:scenarios:regression` and `corepack pnpm test:regression:opencode` pass before claiming recent OpenCode worksheet-operation regressions are fixed. The regression scenario gate includes reloaded `.xlsx` artifact assertions from the MCP-applied fake-host workbook and a no-active-workbook recovery check; desktop Excel save/export fidelity still requires `corepack pnpm test:e2e:live:*`.
- `corepack pnpm test:e2e:scenarios:departments` passes before claiming deterministic department workflow coverage for finance, sales/ops, logistics, HR, executive reporting, or data cleanup.
- After the remaining stricter lanes from `docs/test-strategy.md` are implemented, expanded `corepack pnpm test:e2e:workbook` coverage must pass before full runtime-applied file-level operation-mutation claims, and the expanded `corepack pnpm test:e2e:scenarios` suite must pass before broad deterministic agent workflow claims.
- MCP callable tool names stay in sync with the public agent tool contract. `corepack pnpm verify` runs `scripts/validate/mcp-surface.mjs` after build and fails if the MCP server exposes anything other than `excel.agent.run`.
- Tool surface docs stay in sync with the internal capability catalog and public one-tool MCP surface. `corepack pnpm verify` runs `scripts/validate/docs-surface.mjs` and fails if `docs/tool-surface.md` omits a stable or preview backend capability.
...
- CLI install smoke stays healthy. `corepack pnpm verify` runs `tests/smoke/cli.mjs` and checks `doctor`, `paths`, setup dry-run output, skills.sh guidance, fallback instruction generation, service wrapper generation, sideload manifest generation, and concise disconnected-runtime errors without requiring Excel or a bound local daemon.
- `git diff --check` passes.
- GitHub Actions CI passes the same non-E2E verification plus `corepack pnpm pack:dry-run` for every publishable package.
- Capability catalog, README, installation docs, MCP client docs, and instruction docs match the public `excel.agent.run` MCP surface.
- Fresh local install can run `npx -y @components-kit/open-workbook setup`, install `open-workbook-skills` with `npx skills add components-kit/open-workbook --skill open-workbook-skills`, generate/install a manifest, launch `npx -y @components-kit/open-workbook@latest mcp`, and report runtime status. If the runtime cannot bind or cannot be reached, CLI commands must fail with concise user-facing errors.
...
- Real Excel smoke starts with `OPEN_WORKBOOK_LIVE_E2E=1 corepack pnpm test:e2e:live:mac` or `OPEN_WORKBOOK_LIVE_E2E=1 corepack pnpm test:e2e:live:windows` and must confirm the backend is reachable and the Excel add-in has an active workbook. For deeper host claims, add `-- --deep` to run the `scratch-core` group with scratch-sheet range writes, formula validation, snapshot diff, and rollback preview. For desktop Excel claims about recent worksheet regressions, run `OPEN_WORKBOOK_LIVE_E2E=1 corepack pnpm test:e2e:live:mac:regression` or `OPEN_WORKBOOK_LIVE_E2E=1 corepack pnpm test:e2e:live:windows:regression`.
