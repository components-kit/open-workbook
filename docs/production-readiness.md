...
- MCP callable tool names stay in sync with the public agent tool contract. `corepack pnpm verify` runs `scripts/validate-mcp-catalog.mjs` after build and fails if the MCP server exposes anything other than `excel.agent.run`.
