...
Use `context.strategy`, `context.scope`, and `context.include` when the request needs a specific acquisition policy such as focused selected-area context, analysis, or audit facets. Use `detailLevel` only for the returned preset shape; use `responseMode` for answer verbosity.
