...
Backend tests are grouped by behavior instead of keeping orchestration and runtime coverage in monolithic files. Agent orchestration tests cover prepare/cache, read/answer routing, target resolution, preview/apply safety, and structured intent routing. Runtime service tests cover persistence, transactions/jobs, capabilities/session readiness, range/table behavior, pivot/chart behavior, backups/native file bridge, and locks/tasks. The capability registry test verifies all 308 cataloged capabilities have exactly one backend registry entry and valid metadata, but it is not operation behavior coverage. Shared fake runtimes and helper fixtures live in `*.test-support.ts` files so new tests can target the closest subsystem without adding new public MCP tools or expanding orchestration coverage by accident.

Add-in host tests currently centralize registry and dispatch metadata checks in `apps/excel-addin/src/host/registry.test.ts`, with version behavior covered separately. Live Office.js behavior remains covered by focused executor tests and e2e workflows as those paths are extracted further. See `docs/test-strategy.md` for the target operation, MCP contract, and scenario lanes.
