...
`excel.pivot.repair_from_template` runs fingerprint comparison, applies deterministic template copy, and compares again. Strict mode blocks when required fingerprint dimensions are unavailable. `excel.pivot.rebuild_with_source` is the safe source-change strategy: create a new PivotTable from the desired source and optionally replay template settings. With `replaceExisting`, it performs an explicit delete/create flow with separate backups and transactions instead of trying unsafe in-place source reassignment.
