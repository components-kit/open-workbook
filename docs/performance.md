...
Snapshot creation and mutation results can contain full before/after workbook payloads for rollback. In compact mode, these responses return metadata, IDs, counts, rollback proof, and a `resourceUri` for the full detail instead of embedding snapshot, backup, or diff bodies inline. Stored detail can be inspected with metadata, preview, or paged resource reads before requesting full payloads.
