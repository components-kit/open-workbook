...
Compact agent responses default to `brief`, storing full details locally and returning `workbookContextId`, `continuation`, and `excel://agent/results/{result_id}` handles plus proof metadata. Responses are guarded by shared output limits for summaries, warnings, examples, issue lists, schema/sample fields, and final result size. Budget-limited responses return result handles for full details plus truncation and telemetry. Mutation results include proof metadata so agents can report changed cells/ranges, warnings, backup IDs, rollback availability, and invalidated handles without reading changed sheets back.
...
Snapshot creation and mutation results can contain full before/after workbook payloads for rollback. Public MCP responses return metadata, IDs, counts, rollback proof, `nextActionRecommendation`, `reasoningHints`, confidence metadata, and result resource handles for full detail instead of embedding snapshot, backup, or diff bodies inline. Stored detail is exposed as MCP resource links returned by `excel.agent.run`; `excel://compact/{resource_id}` remains a compatibility alias for stored agent results.

For retry-prone mutation paths, pass a stable `idempotencyKey`. Replays with the same key and payload return `idempotentReplay: true` without applying the edit again; the same key with a different payload is rejected. Successful mutations also invalidate stale workbook-local context/result resources and report `invalidatedContextIds` plus `invalidatedResourceUris`, preventing old read or validation context from being reused after an edit.
