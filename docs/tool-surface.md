...
Vertical OCR/reconciliation prompts are intentionally unsupported for now because OCR and vertical reconciliation workflows are explicit non-goals. They return guidance to use generic table, range, template, validation, and cleaning tools with user-provided data instead.
...
PivotTable and chart tools use native Office.js APIs. Pivot creation supports source ranges or structured tables, destination cells, row/column/filter/data field layout, aggregation, number formats, and layout flags. Pivot reads include layout and hierarchy metadata where Office.js exposes it. Pivot validation can check expected source fields and expected row/column/filter/data-axis placement before an agent mutates a report. Pivot template copy replays deterministic options, layout, hierarchy order, data aggregation/number formats, and basic field settings through a transaction-backed path when the target pivot has matching source fields. Chart creation/update supports source range, chart type, series orientation, title, position, and style. Chart template copy replays deterministic chart metadata such as type, style, title, and geometry through a transaction-backed path. Pivot source reassignment still returns capability-status metadata because Office.js does not expose a safe deterministic operation.
