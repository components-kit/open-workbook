...
Use `context` to guide how the backend gathers context, and `detailLevel` to choose the returned preset shape. `context.strategy` explains why context is needed (`overview`, `focused`, `analysis`, or `audit`), `context.scope` says where to start (`active_selection`, `active_region`, `active_sheet`, `target`, or `workbook`), and `context.include` lists important facets such as `metadata`, `schema`, `values`, `field_context`, `formulas`, `formats`, `validation`, and `filters`. `detailLevel` does not by itself mean “read deeper”; it shapes the result once the router has chosen a safe acquisition policy:
