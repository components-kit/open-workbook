...
- `values`: canonical mutation patches, formulas, styles, table rows, template data, backup IDs, or confirmation data.

For workbook mutations, do not put the write payload only in `request` and do not use top-level `values.values`. Send `values.patches` for every direct range mutation, including single-cell edits. Each patch owns its mutation `target`; top-level `target` is only read/context/default scope. Table appends use `values.rows`.
...
Use `context` and `detailLevel` separately. `context.strategy/scope/include` guides why and where Open Workbook gathers context (`overview`, `focused`, `analysis`, or `audit`; selected area, target, sheet, or workbook; facets such as values, field context, formulas, formats, validation, or filters). `detailLevel` is only the returned preset shape: `workbook_summary` for broad overview, `semantic_index` for role-aware workbook targets/candidates, `sheet_summary` for sheet overview, `table_sample` for a bounded table sample, and `full_table` only when the task requires all rows, every value, or full table contents.
...
- Read-only row lookup: when the user asks to show/list/find rows where a condition is true, call `intent.action: "query_rows"` with `values.where`, optional `values.return`, `values.limit`, and `values.format`. Use `filter_range` only when the user explicitly asks to change the visible Excel filter.
...
- Dropdowns and allowed values: call `intent.action: "read_data_validation"` on the selected/current column or exact target before guessing from visible values. When it returns `answer.kind: "data_validation_summary"`, answer from the inline validation metadata/options and stop; do not fetch `fullResultUri`, chunk-read sheets, list MCP resources, or read raw rows unless the user explicitly asks for raw audit metadata. If the user asks to read values from a source-list sheet such as `Dropdown Lists`, read the actual cell values with `read_values` or a targeted range read; do not treat the sheet name itself as validation intent. When dropdown options are wrong, read validation/source-list proof first, then use `intent.action: "update_dropdown_options"` with `values.operation` (`add`, `rename`, `delete`, `replace_all`, or `reorder`). If the dropdown source/rule itself must change, including mixed or inconsistent existing validation, use one `preview_update` with `intent.action: "write_data_validation"` and one `apply_update`; Open Workbook replaces the existing rule. Do not retry source formula variants, fetch resources, or test-write cells. If the fresh apply fails, report the exact error and stop.
- Filters, borders, dropdowns, and conditional formatting: send canonical intent early. Use `intent.action: "filter_range"` to add autofilter controls to all columns in a resolved range and for structured table criteria filters, `clear_table_filters` for table filter removal/full-data view, `sort_table` for one structured-table sort, `apply_table_view` for combined clear/filter/sort table view changes, `format_range` for borders/fills/fonts/alignment, `write_data_validation` for dropdown/list cells, and `write_conditional_formatting` for formula-based formatting. Ordinary ranges support enabling/clearing autofilter controls here; criteria-based filtering is table-scoped. Do not retry as sheet creation or value writes when the target sheet already exists.
