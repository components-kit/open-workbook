...
Treat returned `resourceLinks`, `compactProof`, `proof`, `continuation`, `invalidatedContextIds`, `invalidatedResourceUris`, `nextAction`, warnings, backup IDs, transaction IDs, and telemetry as evidence. Reuse `workbookContextId` or the returned `continuation` on follow-up `excel.agent.run` calls. `excel://...` handles are MCP/Open Workbook handles, not web URLs; never use `webfetch` for them. If full detail is needed, call `excel.agent.run` again with the returned `resultUri` or `fullResultUri` in `request` or `continuation`. If `nextAction` says `answer_now`, answer from the proof and inline preview instead of fetching more detail unless the user explicitly requested an audit, full rows, or raw values.
