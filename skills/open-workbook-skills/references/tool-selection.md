...
- `auto`: default route for casual prompts and small exact value edits; explicit modes are still preferred for risky, structural, formula, style, template, or table workflows.
...
- Use `mode: "auto"` for small exact value edits the user already asked you to make: explicit `target.range` or per-patch ranges, explicit `values`, no formula-like values, and no broad/structural/style/template/table intent. Leave `autoApply` unset so the backend can auto-apply after workbook write access is allowed for the session.
- Use `mode: "preview_update"` for user-reviewable edits, large or ambiguous changes, formulas, styles/templates, table appends/updates, structural workbook actions, safety artifact lifecycle changes, or when the user asks to review first.
...
- If `preview_ready` is returned, ask/apply once with the returned token and do not rediscover workbook context.
- For `write_values`, send `values.patches`, where each patch has `target.sheetName`, `target.range`, and a 2D `values` matrix. Row data embedded only in `request` is not used for safe writes. Do not use top-level `values.values` for direct workbook mutations.
- For related small exact range edits, send one `mode: "auto"` call with `values.patches`. Use preview/apply only when the grouped edit is broad, risky, ambiguous, or reviewable.
- Multiple Updates: if one user prompt gives multiple explicit value edits, group them in one `values.patches` payload even when they touch different rows or topics. Different topic does not mean different tool call; same user instruction plus explicit ranges means one grouped patch. Do not issue parallel or sequential update calls unless the grouped call fails with actionable details.
- For broad deterministic column/range changes, use `intent.action: "transform_values"` with operations such as `add_prefix`, `add_suffix`, `replace_text`, `normalize_whitespace`, `case`, `fill_blank`, or `map_values`; do not read full columns and generate a giant write matrix.
- For row-aware source-to-target updates, use `intent.action: "derive_values"` with `sourceColumn`/`sourceRange`, `targetColumn`/`target.range`, and an operation such as `copy_from_source`, `copy_if_blank`, `extract_pattern`, `conditional_map`, `lookup_map`, `normalize_from_source`, or `formula_like` for calculations such as Payment Variance = Actual Amount - Cash Amount.
- For workbook structure batches such as adding a prefix/suffix to many sheet names, use `intent.action: "transform_sheets"` with `values.operation`, `values.prefix`/`values.suffix`, and optional `values.sheets`; do not loop over sheet-by-sheet rename calls.
- For range inspection, use `read_range_compact`, `get_range_summary`, `read_hyperlinks`, `read_comments`, `read_notes`, `read_merged_cells`, `read_data_validation`, `read_conditional_formatting`, `search_range`, `find_blank_cells`, or `find_range_errors` in answer mode with a concrete range. For historical labeling/classification examples or “look at another sheet/month for reference” requests, use `find_similar_rows` with the current row/range/table or requested reference sheet; Open Workbook returns exact matched rows with sheet/row proof, so do not manually read broad prior sheets, chase `fullResultUri`, or chunk rows.
- For formulas, use `intent.action: "read_formulas"` for exact formula proof and `intent.action: "write_formulas"` with formula matrices beginning with `=` for writes. For formula-based cell coloring or row highlighting, use `intent.action: "write_conditional_formatting"` with `values.formula` or `values.rule.formula` plus `values.style`; do not route conditional-format rules through `write_formulas`.
...
- For sheet structure and presentation, use `create_sheet`, `copy_sheet`, `rename_sheet`, `hide_sheet`, `unhide_sheet`, or `set_sheet_tab_color`; pass `target.sheetName` for existing sheets, `values.newSheetName` for copy/rename, `values.sheetName` for create, and `values.color` for tab color. For many sheet renames from one deterministic rule, prefer `transform_sheets`. Use raw `copy_sheet` only for non-template exact duplicates including old values.
...
- For “create from template,” prefer a clean template copy: preserve headers, formulas, styles, validation, and layout, then clear old data regions so fresh rows follow the same pattern. If the user explicitly asks to keep/include/preserve data or wants an exact/raw template duplicate, still use `copy_template_sheet` and pass `values.copyMode: "with_data"` or `values.includeData: true`.
