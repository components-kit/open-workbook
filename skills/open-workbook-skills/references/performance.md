...
- For dropdown option inspection, treat `data_validation_summary` as complete inline proof. Do not spend follow-up calls on `fullResultUri` unless the user requested raw audit metadata. Option add/rename/delete/replace/reorder work should use `update_dropdown_options`; use one dropdown-rule preview/apply only when changing the validation rule itself.
