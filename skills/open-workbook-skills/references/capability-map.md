...
- Target discovery: `find_target`, `find_similar_rows`, `find_style_references`, `read_schema`, `get_range_summary`, `read_range_compact`.
...
- Values/formulas/formats: `write_values`, grouped `values.patches`, backend-compiled `transform_values`, `derive_values` including `formula_like`, and `settle_reconciliation`, `write_formulas`, `write_number_formats`, `format_range`, `write_styles_many`, `write_data_validation`, `update_dropdown_options`, `write_conditional_formatting`, `clear_style_dimensions`, grouped internal `write_values_many`/`write_number_formats_many`/`clear_many`/`clear_formats_many`/`clear_style_dimensions_many`/`autofit_many`, `clear_values`, `clear_values_raw`, `clear_formats`, `copy_range`, `move_range`.
- Structure: `insert_rows`, `delete_rows`, `insert_columns`, `delete_columns`, `reorder_range_columns`, `merge_range`, `unmerge_range`, `create_sheet`, `copy_sheet`, `rename_sheet`, backend-compiled `transform_sheets`, `delete_sheet`, `hide_sheet`, `unhide_sheet`, `protect_sheet`, `unprotect_sheet`, `clear_sheet`, `set_sheet_tab_color`, `autofit`, `autofit_rows`.
...
- Template workflows: `copy_template_sheet` backed by internal `sheet.copy_clean_data_regions` for clean copies or `sheet.copy` when `values.copyMode: "with_data"`/`values.includeData: true`, `register_template`, `unregister_template`, `clear_template_data_regions`, `fill_template_regions`, `validate_sheet_against_template`, `repair_sheet_from_template`, `create_template_report`.
- Formula workflows: `create_formula_sheet`, `read_formulas`, `read_formula_patterns`, `get_formula_dependency_graph`, `trace_formula_precedents`, `trace_formula_dependents`, `validate_formula_range`, `validate_formula_against_template`, `find_formula_errors`, `explain_formula`, `copy_formula_patterns`, `fill_formula_down`, `fill_formula_right`, `repair_formula_patterns`, `convert_formulas_to_values`, `recalculate_formulas`, `repair_formulas_from_template`.
